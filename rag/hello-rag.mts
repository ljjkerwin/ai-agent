...
for (const question of questions) {
    ...
    /**
     * 总结两者的关系
retriever.invoke = 检索到的 Document[]（过滤/丢弃了 score 分数，方便在 LangChain Runnable 链中无缝传输）。
vectorStore.similaritySearchWithScore = [Document, score][]（保留了相似度得分，方便你在代码中做分数阈值过滤、日志打印或评分计算）。
所以你的理解非常准确：底层都是同一个向量搜索，结果文档一模一样，只是 retriever 帮你抹去了得分这一项。
     */
    ...
    retrievedDocs.forEach((doc, i) => {
        ...
        const score = scoredResult ? scoredResult[1] : 0;
        const similarity = score !== null ? (1 - score).toFixed(4) : 'N/A'

        console.log(`\n[文档 ${i + 1}] 相似度: ${similarity}`);
        // console.log(`内容: ${doc.pythonpageContent}`);
    })

    const context = retrievedDocs.map((doc, i) => `[文档 ${i + 1}]\n${doc.pageContent}`).join('\n\n------\n\n')
