...
import getRedis from "../../utils/getRedis.mts";
import getModel from "../../utils/getModel.mts";

const REDIS_HOST = process.env.REDIS_HOST ?? "localhost";
const REDIS_PORT = Number(process.env.REDIS_PORT ?? 6379);
const REDIS_DB = Number(process.env.REDIS_DB ?? 0);
const MEMORY_TTL = Number(process.env.MEMORY_TTL_SECONDS ?? 18000);
...
class Mem0MemoryStore {
  ...
  buildSystemMessage({ user, session }) {
    console.log('长期记忆:\n', user.concat(session).map(m => m.memory).join("----\n"))
    const blocks = [];
    if (user.length) {
...
//   console.log('memoryMsg', memoryMsg)
...
const classifier = getModel().withStructuredOutput(memorySchema);
